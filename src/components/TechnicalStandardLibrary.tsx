...
export interface TechnicalStandardLibraryProps {
...
const TechnicalStandardLibrary: React.FC<TechnicalStandardLibraryProps> = ({ 
  selectedStandard, 
  onStandardSelect, 
  technicalStandards = [] 
}) => {
        ...
        {technicalStandards.map((standard) => (
          ...
          >
            ...
            <div className="flex items-center justify-between text-xs mt-1">
              <span className="text-gray-500">{standard.creator}</span>
              <span className={`px-2 py-1 rounded ${
                standard.status === '已发布' ? 'bg-green-100 text-green-700' :
                standard.status === '评审中' ? 'bg-yellow-100 text-yellow-700' :
                standard.status === '草案' ? 'bg-gray-100 text-gray-700' :
